 ...
 *
 * Shows title, summary text, and delete button (always visible with red bg).
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h4 className="text-sm font-semibold text-feldora-text truncate">
              {bill.title}
            </h4>
            <p className="text-xs text-feldora-text-secondary mt-0.5 truncate">
              {bill.summary}
            </p>
          </div>
          <span className="text-[10px] text-feldora-muted font-mono shrink-0 mt-0.5">
            {formatRelativeDate(bill.updatedAt)}
          </span>
        </div>
      </Link>

      {/* Delete button — always visible, red bg */}
      <button
        type="button"
        onClick={(e) => {
          e.stopPropagation()
