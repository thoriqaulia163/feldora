...
import { buildRawFeatureVector, getLocationIdx } from './weatherUtilsV3NN'
...
export function useNNWeatherPredictor(): PredictorHook {
  ...
  const loadModel = useCallback(async () => {
    ...
    try {
      const res = await fetch('/ai-models/local-weather-forecast-v3/model.json')
