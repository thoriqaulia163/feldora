...
/**
 * Build raw (un-normalized) numeric feature vector for V3 NN.
 * 14 features: [dayOfYear, lat, lng, elev, monsoonZone, localSeasonIndex,
 *               enso, iod, prevDayRain, sinDay, cosDay, sinMonth, cosMonth, dayLength]
 *
 * The caller must then normalize using model.normalization before inference.
 */
export function buildRawFeatureVector(
  ...
  const featureMap: Record<string, number> = {
    ...
    sinMonth: Math.round(sinMonth * 1000) / 1000,
    cosMonth: Math.round(cosMonth * 1000) / 1000,
...
/**
 * Get location index from model vocabulary.
