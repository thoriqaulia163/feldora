/**
 * Curtain/slider comparison — identical to quick-upscaler version
...
export function SliderComparison({
  originalURL, resultURL, resultWidth, resultHeight, position, onPositionChange,
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {/* Result — base layer */}
        <img src={resultURL} alt="Upscaled result"
        ...
        <span className="absolute top-2 left-2 font-mono text-[9px] uppercase tracking-widest text-white/70 bg-black/40 px-1.5 py-0.5 pointer-events-none">Original</span>
        <span className="absolute top-2 right-2 font-mono text-[9px] uppercase tracking-widest text-white/70 bg-black/40 px-1.5 py-0.5 pointer-events-none">Result</span>
      </div>
      <p className="text-feldora-muted font-mono text-[10px] text-center">
        {resultWidth} × {resultHeight} px — drag divider to compare
      </p>
    </div>
  )
}
