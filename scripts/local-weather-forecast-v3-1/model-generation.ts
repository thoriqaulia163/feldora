/**
 * Local Weather Forecast V3.1 — Neural Network Training Script
 *
 * Improvements over V3:
 * - Larger network: [128, 64, 32] (was [64, 32, 16])
 * - Larger embedding: dim=16 (was dim=8)
 * - Learning curves saved per epoch
 * - Rolling temporal validation
 * - Same feature set as V3 (isolated architecture improvement)
 *
 * Usage: npx tsx scripts/local-weather-forecast-v3-1/model-generation.ts
 */
...
const CONFIG = {
  // Architecture (V3.1: larger capacity)
  embeddingDim: 16,
...
function buildNumericFeatures(row: DataRow): number[] {
  ...
  if (cosHA > 1) dayLength = 0
  else if (cosHA < -1) dayLength = 24
  else dayLength = (2 * Math.acos(cosHA) * 180 / Math.PI) / 15
  const prevDayRain = row.prevDayRainSlots >= 1 ? 1 : 0

  return [
    row.dayOfYear, row.latitude, row.longitude, row.elevation, row.monsoonZone,
    row.localSeasonIndex, row.enso, row.iod, prevDayRain,
    sinDay, cosDay, sinMonth, cosMonth, dayLength,
  ]
}
...
function initWeights(vocabSize: number, embDim: number, inputDim: number, hidden: number[], nHeads: number, rng: () => number): NNWeights {
  ...
  const embedding: number[][] = Array.from({ length: vocabSize }, () => Array.from({ length: embDim }, () => xavier(vocabSize, embDim)))
  const layers: { w: number[][]; b: number[] }[] = []
  let prev = inputDim
  for (const size of hidden) {
    const w = Array.from({ length: size }, () => Array.from({ length: prev }, () => xavier(prev, size)))
    layers.push({ w, b: new Array(size).fill(0) })
    prev = size
  }
  const heads: { w: number[][]; b: number[] }[] = []
  for (let h = 0; h < nHeads; h++) {
    heads.push({ w: [Array.from({ length: prev }, () => xavier(prev, 1))], b: [0] })
  }
...
function forward(weights: NNWeights, locIdx: number, features: number[]): number[] {
  ...
  for (const layer of weights.layers) {
    const out: number[] = new Array(layer.w.length)
...
function backprop(weights: NNWeights, locIdx: number, features: number[], targets: number[], grads: Gradients): number {
  ...
  for (const layer of weights.layers) {
    const pre: number[] = []; const out: number[] = []
    for (let i = 0; i < layer.w.length; i++) {
  ...
  for (let l = weights.layers.length - 1; l >= 0; l--) {
    const inp = allInputs[l]; const pre = allPre[l]; const layer = weights.layers[l]
...
function initAdam(w: NNWeights): AdamState {
  return {
    mE: w.embedding.map(r => new Array(r.length).fill(0)),
    vE: w.embedding.map(r => new Array(r.length).fill(0)),
...
function adamStep(w: NNWeights, g: Gradients, st: AdamState, lr: number, bs: number): void {
  ...
  for (let i = 0; i < w.embedding.length; i++)
    for (let j = 0; j < w.embedding[i].length; j++) {
      ...
      const [p, m, v] = upd(w.embedding[i][j], g.embedding[i][j], st.mE[i][j], st.vE[i][j])
  ...
  for (let l = 0; l < w.layers.length; l++) {
    for (let i = 0; i < w.layers[l].w.length; i++) {
      for (let j = 0; j < w.layers[l].w[i].length; j++) {
        const [p, m, v] = upd(w.layers[l].w[i][j], g.layers[l].dw[i][j], st.mL[l].mw[i][j], st.vL[l].vw[i][j])
        w.layers[l].w[i][j] = p; st.mL[l].mw[i][j] = m; st.vL[l].vw[i][j] = v
      }
      const [pb, mb, vb] = upd(w.layers[l].b[i], g.layers[l].db[i], st.mL[l].mb[i], st.vL[l].vb[i])
  ...
  for (let h = 0; h < w.heads.length; h++) {
    for (let j = 0; j < w.heads[h].w[0].length; j++) {
      const [p, m, v] = upd(w.heads[h].w[0][j], g.heads[h].dw[0][j], st.mH[h].mw[0][j], st.vH[h].vw[0][j])
      w.heads[h].w[0][j] = p; st.mH[h].mw[0][j] = m; st.vH[h].vw[0][j] = v
    }
    const [pb, mb, vb] = upd(w.heads[h].b[0], g.heads[h].db[0], st.mH[h].mb[0], st.vH[h].vb[0])
...
function computeMacroF1(weights: NNWeights, samples: PreparedSample[], thresholds: number[]): number {
  ...
  for (const s of samples) {
    ...
    for (let h = 0; h < 4; h++) {
      ...
      if (pred === 1 && act === 1) tp[h]++
      else if (pred === 1 && act === 0) fp[h]++
      else if (pred === 0 && act === 1) fn[h]++
      else tn[h]++
    }
  }
  let sum = 0
...
function evaluateWithThresholds(weights: NNWeights, samples: PreparedSample[], thresholds: number[]): { overall: { accuracy: number; macroF1: number }; perSlot: SlotMetrics[] } {
  const tp = [0,0,0,0], fp = [0,0,0,0], fn = [0,0,0,0], tn = [0,0,0,0]
  for (const s of samples) {
    const p = forward(weights, s.locationIdx, s.numericFeatures)
    for (let h = 0; h < 4; h++) {
  ...
  const perSlot: SlotMetrics[] = []; let totCorr = 0, totN = 0, macroSum = 0
  for (let h = 0; h < 4; h++) {
    ...
    totCorr += tp[h]+tn[h]; totN += tp[h]+fp[h]+fn[h]+tn[h]; macroSum += slotF1
  }
  return { overall: { accuracy: totCorr/totN, macroF1: macroSum/4 }, perSlot }
}
...
function evaluateLoss(weights: NNWeights, samples: PreparedSample[]): number {
  ...
  for (const s of samples) {
    const p = forward(weights, s.locationIdx, s.numericFeatures)
    for (let h = 0; h < 4; h++) {
      const pr = Math.max(1e-7, Math.min(1 - 1e-7, p[h]))
      total -= s.targets[h] * Math.log(pr) + (1 - s.targets[h]) * Math.log(1 - pr)
    }
  }
...
function train(weights: NNWeights, trainS: PreparedSample[], valS: PreparedSample[]): { best: NNWeights; history: EpochLog[] } {
  ...
  for (let epoch = 1; epoch <= CONFIG.maxEpochs; epoch++) {
    ...
    for (let b = 0; b < trainS.length; b += CONFIG.batchSize) {
      ...
      adamStep(weights, grads, adam, CONFIG.learningRate, end - b)
    }
...
function rollingValidation(data: DataRow[], vocab: Map<string, number>): void {
  console.log('\n── Rolling Temporal Validation ──')
  const configs = [
    { trainYears: [2021, 2022], valYear: 2023, label: 'Train 21-22, Val 23' },
    { trainYears: [2021, 2022, 2023], valYear: 2024, label: 'Train 21-23, Val 24' },
  ]

  for (const cfg of configs) {
    const trainData = data.filter(r => cfg.trainYears.includes(r.year))
    const valData = data.filter(r => r.year === cfg.valYear)
    const trainSamples = prepareSamples(trainData, vocab)
    const valSamples = prepareSamples(valData, vocab)
    const stats = computeNormStats(trainSamples)
    normalizeSamples(trainSamples, stats)
    normalizeSamples(valSamples, stats)

    const rng = mulberry32(CONFIG.seed)
    const inputDim = CONFIG.embeddingDim + FEATURE_NAMES.length
    const w = initWeights(vocab.size, CONFIG.embeddingDim, inputDim, CONFIG.hiddenLayers, 4, rng)
    const adam = initAdam(w)
    const rng2 = mulberry32(CONFIG.seed + 2000)

    // Quick training (same hyperparams, no logging)
    let bestLoss = Infinity, bestW = JSON.stringify(w), pat = 0
    for (let ep = 1; ep <= CONFIG.maxEpochs; ep++) {
      const idx = shuffleIndices(trainSamples.length, rng2)
      for (let b = 0; b < trainSamples.length; b += CONFIG.batchSize) {
        const end = Math.min(b + CONFIG.batchSize, trainSamples.length)
        const grads = zeroGrads(w)
        for (let i = b; i < end; i++) { const s = trainSamples[idx[i]]; backprop(w, s.locationIdx, s.numericFeatures, s.targets, grads) }
        adamStep(w, grads, adam, CONFIG.learningRate, end - b)
      }
      const vl = evaluateLoss(w, valSamples)
      if (vl < bestLoss) { bestLoss = vl; bestW = JSON.stringify(w); pat = 0 } else { pat++; if (pat >= CONFIG.earlyStoppingPatience) break }
    }
    const bestWeights: NNWeights = JSON.parse(bestW)
    const f1 = computeMacroF1(bestWeights, valSamples, [0.5, 0.5, 0.5, 0.5])
    console.log(`  ${cfg.label}: val_loss=${bestLoss.toFixed(4)}, Macro F1 (t=0.5)=${(f1*100).toFixed(2)}%`)
  }
}
...
function exportModel(weights: NNWeights, vocab: Map<string, number>, normStats: NormStats, thresholds: number[], history: EpochLog[], trainSize: number, valSize: number, testSize: number): void {
  ...
  const artifact = {
    ...
    metadata: {
      ...
      datasetSize: trainSize + valSize + testSize,
  ...
  const curvePath = resolve(process.cwd(), CONFIG.reportDir, 'learning-curves.json')
...
function main() {
  console.log('═══════════════════════════════════════════════════════════')
  console.log(' Local Weather Forecast V3.1 — Neural Network Training')
  console.log('═══════════════════════════════════════════════════════════\n')
  ...
  console.log('\n═══════════════════════════════════════════════════════════')
  console.log(' V3.1 Training complete!')
  console.log('═══════════════════════════════════════════════════════════')
}
